...
async def add_service(request):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        sql = f"CREATE TABLE IF NOT EXISTS {service} (ip CHAR(16), port INT, heartbeat CHAR(50), alive  BOOL, UNIQUE(ip, port));"
...
async def get_one_service(request):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        await db.commit()
        fetched = await resp.fetchall()
    rando = random.choice(fetched)
    services = {'endpoints': rando}
    return web.json_response(services)
...
def create_db(db_name):
    ...
    cur = conn.cursor()
    sql = "CREATE TABLE services (name CHAR(50));"
    cur.execute(sql)
    conn.commit()
    conn.close()
